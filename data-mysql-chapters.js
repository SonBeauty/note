...
window.MYSQL_CHAPTERS = [
  {
    ...
    blocks: [
      ...
      { t: "p", html: "<b>Bước 1 — mỗi vế đang làm gì.</b> Giả sử hôm nay là 15/09/2026 và một dòng có hạn trả là <code>2026-08-01 00:00:00</code>:" },
      { t: "table", head: ["Biểu thức","Kết quả","Kiểu"], rows: [["<code>x.PlannedRepaymentDate</code>","<code>2026-08-01 00:00:00</code>","ngày giờ"],["<code>LEFT(..., 10)</code>","<code>'2026-08-01'</code>","chuỗi, lấy 10 ký tự đầu"],["<code>CURDATE()</code>","<code>2026-09-15</code>","ngày"],["<code>DATE_FORMAT(..., '%Y-%m-%d')</code>","<code>'2026-09-15'</code>","chuỗi"]] },
      { t: "p", html: "Hai vế đều bị biến thành <b>chuỗi chữ</b>, rồi so <code>'2026-08-01' &lt; '2026-09-15'</code> như so hai từ trong từ điển." },
      { t: "p", html: "<b>Bước 2 — vì sao vẫn ra đúng.</b> So chuỗi là so từng ký tự từ trái qua. Định dạng <code>năm-tháng-ngày</code> đặt phần lớn nhất lên đầu, nên so chữ cũng chính là so ngày:" },
      { t: "code", text: "'2026-08-01' vs '2026-09-15'\n 2026 = 2026  → so tiếp\n   08 < 09    → dừng, vế trái nhỏ hơn  ✓ đúng" },
      { t: "p", html: "Chỉ may mắn nhờ định dạng. Đổi sang kiểu Việt Nam <code>ngày/tháng/năm</code> là sai ngay:" },
      { t: "code", text: "'01/08/2026' vs '15/09/2026'\n  0 < 1       → vế trái nhỏ hơn  ✓ tình cờ đúng\n\n'20/08/2026' vs '15/09/2026'\n  2 > 1       → vế trái LỚN hơn  ✗ sai, 20/08 rõ ràng trước 15/09" },
      { t: "p", html: "<b>Bước 3 — vì sao chậm.</b> Index giống danh bạ điện thoại đã xếp sẵn theo ngày hạn trả. Tìm <i>những dòng trước 15/09</i> thì mở danh bạ lật tới đó là xong, khỏi đọc từng trang." },
      { t: "p", html: "Nhưng danh bạ được xếp theo <b>giá trị gốc của cột</b>, không xếp theo <code>LEFT(cột, 10)</code>. Bọc cột trong hàm là bắt MySQL tính lại hàm đó cho <b>từng dòng một</b> rồi mới so, nên danh bạ vô dụng, phải đọc cả bảng. Bảng 10 dòng thì không sao, bảng 5 triệu dòng thì truy vấn từ vài mili giây thành vài giây." },
      { t: "hint", html: "Kiểu điều kiện làm mất index như vậy gọi là <b>non-sargable</b>. Mẹo nhận ra: nhìn vào <code>WHERE</code>, thấy <b>tên cột nằm bên trong một hàm</b> là gần như chắc chắn mất index." },
      { t: "p", html: "<b>Bước 4 — sửa.</b> Để cột đứng một mình, so thẳng với ngày:" },
      { t: "code", text: "x.PlannedRepaymentDate < CURDATE()" },
      { t: "p", html: "Không cắt chuỗi, không đổi định dạng, không phụ thuộc may mắn, và dùng được index. Cột <code>DATETIME</code> so với <code>CURDATE()</code> thì MySQL hiểu <code>CURDATE()</code> là <code>2026-09-15 00:00:00</code>, nên kết quả vẫn là <i>trước ngày hôm nay</i>." },
      { t: "hint", html: "Cách sửa này chỉ đúng khi cột thật sự là kiểu <code>DATE</code> hoặc <code>DATETIME</code>. Nếu cột đang lưu dưới dạng <code>VARCHAR</code> thì <code>LEFT()</code> là bắt buộc, và lỗi thật nằm ở thiết kế bảng chứ không nằm ở câu truy vấn." },
